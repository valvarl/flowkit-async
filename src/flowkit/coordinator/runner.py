...
class Coordinator:
    ...
    async def _preflight_and_maybe_start(self, task_doc: Dict[str, Any], node: Dict[str, Any]) -> None:
        ...
        try:
            cnt = await self.db.artifacts.count_documents({"task_id": task_id, "node_id": node_id, "status": "complete"})
            if cnt > 0:
    ...
    async def _heartbeat_monitor(self) -> None:
        try:
            while True:
                cur = self.db.tasks.find({"status": {"$in": [RunState.running, RunState.deferred]}},
    ...
    async def _finalize_nodes_and_tasks(self) -> None:
        cur = self.db.tasks.find({"status": {"$in": [RunState.running, RunState.deferred, RunState.cancelling]}},
    ...
    async def _ensure_indexes(self) -> None:
        try:
            ...
            await self.db.artifacts.create_index([("task_id", 1), ("node_id", 1), ("batch_uid", 1)], unique=True, sparse=True, name="uniq_artifact_batch")
