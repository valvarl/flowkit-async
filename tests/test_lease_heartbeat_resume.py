...
from tests.helpers import dbg, wait_task_finished
...
from tests.helpers.graph import prime_graph, make_graph, node_by_id, wait_task_status
...
async def test_heartbeat_soft_deferred_then_recovers(env_and_imports, inmemory_db, coord, worker_factory):
    ...
    async def saw_deferred(timeout=3.0) -> bool:
...
@pytest.mark.cfg(coord={"heartbeat_soft_sec": 0.2, "heartbeat_hard_sec": 0.5},
                 worker={"hb_interval_sec": 10.0})
@pytest.mark.asyncio
...
async def test_resume_inflight_worker_restarts_with_local_state(env_and_imports, inmemory_db, coord, worker_cfg,
    ...
    w1 = wu.Worker(db=inmemory_db, cfg=worker_cfg, roles=["sleepy"],
    ...
    w2 = wu.Worker(db=inmemory_db, cfg=worker_cfg, roles=["sleepy"],
...
@pytest.mark.cfg(coord={"discovery_window_sec": 1.0, "scheduler_tick_sec": 0.05,
...
@pytest.mark.cfg(coord={"discovery_window_sec": 5.0, "scheduler_tick_sec": 0.05,
