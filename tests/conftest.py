...
def pytest_configure(config):
    ...
    config.addinivalue_line("markers", "e2e: end-to-end tests exercising scheduler and Kafka mocks")
    config.addinivalue_line("markers", "integration: integration tests")
    config.addinivalue_line("markers", "unit: unit tests")
    config.addinivalue_line("markers", "adapters: input adapters & routing tests (orchestrator-first contract)")
    config.addinivalue_line("markers", "slow: slow tests (can be skipped via -m 'not slow')")
